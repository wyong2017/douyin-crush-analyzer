<p align="center">
  <img src="assets/banner.svg" alt="抖音 Crush 分析器" width="800"/>
</p>

<p align="center">
  <strong>暗恋一个人，只有 TA 的抖音？交给 AI。</strong>
</p>
...
把 `SKILL.md` 和 `references/` 放到 Openclaw 的 Skills 目录：

```
~/.claude/skills/douyin-crush-analyzer/
├── SKILL.md
└── references/
...
```
帮我分析这个抖音 https://www.douyin.com/user/profile/xxxxx
```

或者更直接：

```
我想追这个人，分析一下 TA 的抖音：[链接]
```
...
```
纯画像分析：https://www.douyin.com/user/profile/xxxxx
```
...
<details>
<summary><strong>不登录能用吗？</strong></summary>

能用，但效果大打折扣。未登录只能看到约 10 条作品，看不了详情和收藏。<br/>
登录后数据量是未登录的 <strong>5-10 倍</strong>，分析质量天差地别。
</details>

<details>
<summary><strong>推荐和喜欢设了私密怎么办？</strong></summary>

自动跳过，报告中标注「xx不可见」。分析照样能做，只是失去了最有价值的数据源，理想型推断的准确度会降低。
</details>
